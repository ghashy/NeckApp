...
{
    ClassDB::bind_method(D_METHOD("calculate_result", "buffer", "samples_count",
                                  "frame_size", "sample_rate", "chord",
...
bool ChordInfo::calculate_result(godot::PackedVector2Array buffer,
                                 ...
                                 int calculation_interval)
{
    ...
    for (int j = 0; j < samples_count; j += frame_size) {
        ...
        if (chroma.isReady()) {
            ...
            heared.push(detector.rootNote, detector.quality,
                                  detector.intervals);
            godot::UtilityFunctions::print(detector.rootNote, " ", detector.quality, " ", detector.intervals);
        }
    }
