...
{
    return p_name == StringName("property_from_list")
        && property_from_list != Vector3(42, 42, 42);
};

bool Example::_property_get_revert(const StringName& p_name,
...
void Example::_bind_methods()
{
    ...
    ClassDB::bind_method(D_METHOD("test_dictionary"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::VECTOR2, "group_subgroup_custom_position"),
...
{
    // You can instance and return a refcounted object like this, but keep in
    // mind that refcounting starts with the returned object and it will be
    // destroyed when all references are destroyed. If you store this pointer
    // you run the risk of having a pointer to a destroyed object.
    return memnew(ExampleRef());
}

Example* Example::test_node_argument(Example* p_node) const
{
    UtilityFunctions::print("  Test node argument called with ",
                            p_node ? String::num(p_node->get_instance_id())
                                   : "null");
    return p_node;
}
...
{
    // This is therefor the prefered way of instancing and returning a
    // refcounted object:
    Ref<ExampleRef> ref;
    ref.instantiate();
...
Variant Example::varargs_func(const Variant** args, GDExtensionInt arg_count,
                              GDExtensionCallError& error)
{
    UtilityFunctions::print("  Varargs (Variant return) called with ",
                            String::num((double)arg_count), " arguments");
    return arg_count;
}

int Example::varargs_func_nv(const Variant** args, GDExtensionInt arg_count,
                             GDExtensionCallError& error)
{
...
void Example::varargs_func_void(const Variant** args, GDExtensionInt arg_count,
                                GDExtensionCallError& error)
{
