...
func handle_keys_pressed(index: int) -> void:
	...
	if key == "Select ALL" or key == "Unselect ALL":
		...
		return
	for idx in selected_keys_popup.item_count:
		if not selected_keys_popup.is_item_checked(idx):
			var all: int = selected_keys_popup.item_count - 1
			selected_keys_popup.set_item_checked(all, false)
			selected_keys_popup.set_item_text(all, "Select ALL")
...
func init_keys_and_alts() -> void:
	...
	# Setup selected keys.
	for i in selected_keys_popup.item_count:
		selected_keys_popup.set_item_checked(i, checked_items[selected_keys_popup.get_item_text(i)])
