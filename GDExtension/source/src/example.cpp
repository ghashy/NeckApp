...
ExampleRef::ExampleRef() {
    ...
    UtilityFunctions::print("ExampleRef ", itos(id),
...
void Example::_bind_methods() {
    ...
    ADD_PROPERTY(PropertyInfo(Variant::VECTOR2, "group_subgroup_custom_position"),
...
ExampleRef* Example::return_extended_ref() const {
    // You can instance and return a refcounted object like this, but keep in mind that
    // refcounting starts with the returned object and it will be destroyed when all references
    // are destroyed. If you store this pointer you run the risk of having a pointer to a
    // destroyed object.
...
Example* Example::test_node_argument(Example* p_node) const {
    UtilityFunctions::print("  Test node argument called with ",
                            p_node ? String::num(p_node->get_instance_id()) : "null");
    return p_node;
}
...
int Example::test_vector_ops() const {
    ...
    arr.push_back(45);
    int ret = 0;
    for (const int32_t& E : arr) { ret += E; }
    return ret;
}

void Example::test_tarray_arg(const TypedArray<int64_t>& p_array) {
    for (int i = 0; i < p_array.size(); i++) { UtilityFunctions::print(p_array[i]); }
}
