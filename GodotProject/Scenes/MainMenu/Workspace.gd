...
func run_cycle() -> void:
	...
	if iter_cur >= iter_max and cur_key_idx >= keys.size():
		execute_finish()
	elif iter_cur >= iter_max:
		if cur_key_idx < keys.size():
			cur_key_idx = cur_key_idx + 1
		else:
			execute_finish()
	iter_cur += 1
...
func get_next_chord() -> TargetChord:
	...
	if iter_cur + 1 >= iter_max and cur_key_idx >= keys.size() - 1 and cur_chord_idx >= chords_arr.size():
